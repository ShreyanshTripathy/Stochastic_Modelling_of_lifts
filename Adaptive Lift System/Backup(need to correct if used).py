...
class DualLiftSystem:
    ...
    def switch_to_oscillation(self, passenger_data):
        if (self.lift_A_population == 0 and self.lift_B_population==0) and not self.picking:
                ...
                # permission = input("Do you want to switch to oscillation? (y/n): ")
                permission = "yes"
                if permission.lower() == "n":
                    ...
                    updated_passener_Data = metro.run_simulation(passenger_data)
                    if metro.already_picked_A:
                        for people in metro.already_picked_A:
                            ...
                            if people not in self.passengers_in_lift_A:
                                self.pending_in_lift_A.append(people)
                    ...
                    self.current_floor_A = metro.current_floor_A
                    self.current_floor_B = metro.current_floor_B
                    ...
                    time.sleep(0.5)
                    print(f"Updated passenger data: {updated_passener_Data}")
                    print(f"Lift_A_position: {metro.current_floor_A}, Lift_B_position: {metro.current_floor_B}")
                    print(f"Pending orders: {metro.pending_orders}")
                    print(f"Lift_A_population: {metro.lift_A_Population}, Lift_B_population: {metro.lift_B_Population}")
                    
                    # permission = input("Do you want to continue the simulation? (y/n): ")
                    permission = "yes"
                    if permission.lower() == "n":
                        sys.exit()
                    else:
                        return updated_passener_Data
    ...
    def switch_to_VIP(self, passenger_data):
        ...
        updated_passener_Data = VIP.run_simulation(passenger_data)
    ...
    def run_simulation(self, passenger_data):
        ...
        while passenger_data or self.pending_orders_A or self.pending_orders_B:
            ...
            if self.picking:
                ...
                if self.time_elapsed >= self.delta_time and self.picking:
            ...
            #this is for the Oscillation system if the mean for all floors are higher than the threshold then the system will swutch    
            ...
            if max(self.current_density) >= self.T_high_VIP and self.time_above_threshold >= self.t_persistence and self.current_mode=="normal":
                print(f"Current density = {self.current_density}. Switching to VIP")
                self.current_mode = "VIP"
                
                print("Turning of picking...")
                time.sleep(0.5)
                self.picking = False
                '''Figure out which lift is going to serve the VIP floor....for now only lift B is going to act as the VIP lift'''
            ...
            if np.mean(self.current_density) >= self.T_high_oscillation and self.time_above_threshold >= self.t_persistence:
                print(f"Current density = {self.current_density}. Switching to Oscillation")
            ...
            if max(self.current_density) >= self.T_high_VIP and self.time_above_threshold >= self.t_persistence:
                print(f"Current density = {self.current_density}. Switching to Oscillation")
...
# filepath = "Dummy"
# Passenger_limit = 8
# T_high_oscillation = 0.3
# T_low_oscillation = 0.2
# current_density = [0]*(num_floors+1)
# delta_time = 5
# T_high_VIP = 0.5
# T_low_VIP = 0.2
# dual_oscillation = DualLiftSystem(
#     current_floor_A,current_floor_B, num_floors,filepath, Passenger_limit,T_high_oscillation,T_low_oscillation,current_density,delta_time,T_high_VIP,T_low_VIP,current_time = 0
# )


# # output_file = "density_snapshots.csv"
# remaining_passenger_data = dual_oscillation.run_simulation(passenger_data)

# print("Simulation complete. Remaining passengers:")
# print(remaining_passenger_data)


# '''No more apparent error: 1-1-25'''