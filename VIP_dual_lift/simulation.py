...
def generate_and_process_data(l, file_path,duration,current_floor_A, current_floor_B, VIP_floor,passenger_limit,num_floors,current_timestamp=0,order_done=[]):
    ...
    try:
        ...
        if permission == "yes":
            elevator = VIPDualSystem(current_floor_A=current_floor_A,current_floor_B=current_floor_B,num_floors=num_floors,filepath=file_path,Passenger_limit=passenger_limit,floor_to_serve=VIP_floor ,current_time=current_timestamp)
        else:
...
def run_multiple_scenarios():
    ...
    traffic_levels = {
        "low_traffic": {
            # 5: [0.05] + [0.002] * 5,   # Ground floor higher than others
    ...
    for traffic_level, floor_configs in traffic_levels.items():
        for num_floors, l in floor_configs.items():
            ...
            VIP_floor = 3
            # Run the simulation
            order_done = generate_and_process_data(current_floor_A=current_floor_A,current_floor_B=current_floor_B, num_floors=num_floors, passenger_limit=passenger_limit, duration=duration, file_path = file_path,l=l, VIP_floor=VIP_floor)
